...
export class SanitizerBuilder {
    ...
    public keepRange(charCodeStart: number, charCodeEnd: number): SanitizerBuilder {
...
export class Sanitizer {
    ...
    public sanitize(input: string): string {
        ...
        for (let charIndex = 0; charIndex < input.length; charIndex++) {
            let ascii = input.charCodeAt(charIndex);
            if (this.ranges.some(range => ascii >= range.charCodeStart && ascii <= range.charCodeEnd)) {
